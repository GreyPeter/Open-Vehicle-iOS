...
struct Provider: TimelineProvider {
  ...
  func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
    ...
    var soc:Double = socVal
    let percentToFull = 100.0 - soc
    var maxRange:Double = 0.0
    var percent = soc / 100.0
    ...
    if charging {
      ...
      if let timeToFull = Double(metrics.metricVal.durationfull) {
        if timeToFull > 0 {
         ...
         print("Percent per Minute = \(percentPerMinute)")
        }
      }
    ...
    print("Entries = \(entries.count) Estimated Range = \(String(format:"%0.2f",range)) Charge = \(String(format:"%0.1f",soc))")
